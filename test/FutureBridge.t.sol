...
contract FutureBridgeTest is Test {
	...
	function schnorrSign(
		...
		bytes32 messageHash,
		uint256 privateKey
	) public view returns (address rAddress, uint256 s) {
		rAddress = vm.addr(k);
	...
	function test_verify() public {
		...
		for (uint256 i = 0; i < 100; i++) {
			bytes32 anyMessageHash = keccak256(abi.encodePacked(i, "any message to be sign"));
			(uint8 parity, uint256 px) = getPubkey(privateKey);
			(address rAddress, uint256 s) = schnorrSign(parity, px, randomK(privateKey), anyMessageHash, privateKey);
			...
			nextPrivateKey = uint256(keccak256(abi.encodePacked(i, privateKey, "next privateKey")));
