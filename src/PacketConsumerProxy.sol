...
contract PacketConsumerProxy is Ownable {
    ...
    function prices(string memory signalId)
        public
        view
        returns (IPacketConsumer.Price memory)
    {
        return packetConsumer.prices(signalId);
    }

    /// @dev Returns the prices for the given `signalIds`, reverting if any do not exist.
    function batchPrices(string[] memory signalIds)
        public
        view
        returns (IPacketConsumer.Price[] memory)
    {
        return packetConsumer.batchPrices(signalIds);
    }
}
