...
contract BandTssBridgeConsumer {
    ...
    /// @dev unpack the result from bandchain
    /// @param payload is the result from bandchain
    /// @return sp is the signal price array
    /// @return timestamp is the timestamp of the result
    function unpackResult(bytes memory payload) private pure returns(SignalPrice[] memory sp, int64 timestamp) {
        (sp, timestamp) = abi.decode(payload, (SignalPrice[], int64));
    }
    ...
    function relayFeedsPriceData(address rAddress, uint256 s, uint64 signingID, bytes memory result) external {
        bytes32 msgHash = keccak256(abi.encodePacked(result));
        require(verify(rAddress, s, msgHash),
            "BandTssBridgeConsumer: Public key update fails, the current circle's signature is invalid.");
